...
void addInodeTail(Inode** head, Inode inode){
	...
	while (last->next_inode != NULL)
		last = last->next_inode;
	last->next_inode = new_inode;
	new_inode->prev_inode = last;
}
...
int save_inode_dir_block_index(int index_index, int inode_index, int db_index, Index index){
	...
	sprintf(filename, "tmp/index%dI%dDB%d.tmp", index_index, inode_index, db_index);
	f = fopen(filename, "w+");
...
int save_index(int index_index, int db_index, Index index){
	...
	sprintf(filename, "tmp/index%dDB%d.tmp", index_index, db_index);
	f = fopen(filename, "w+");
	...
	fprintf(f, "%s %d", index.name, index.inode->index);
...
int save_data_blocks(Data_block* blocks){
	...
	int sum = 0;
	while (current != NULL){
		sprintf(filename, "tmp/datablocks%d.tmp", (++sum));
		f = fopen(filename, "wb+");
		if (f == NULL){
			free(filename);
			return 1;
...
int load_data_blocks(Data_block** blocks, int nb_data_blocks){
	if (blocks != NULL){
		...
		for (int i = 0; i < nb_data_blocks; i++){
			sprintf(filename, "tmp/datablocks%d.tmp", (i+1));
			f = fopen(filename, "rb");
			if (f == NULL){
				free(filename);
				return 1;
...
int load_inode_data_blocks(int inode_index, Data_block** blocks, int nb_data_blocks){
	if (nb_data_blocks > 0){
		...
		for (int i=0; i < nb_data_blocks; i++){
			sprintf(filename, "tmp/datablocks%dI%d.tmp", i, inode_index);
			f = fopen(filename, "rb");
			if (f == NULL){
				free(filename);
				return 0;
...
int load_inode_dir_blocks(Inode* inode, int inode_index, Directory_block** blocks){
	...
	for (int i=0; i < inode->nb_dir_blocks; i++){
		sprintf(filename, "tmp/dirblocks%dI%d.tmp", (i+1), inode_index);
		...
		if (read_inode_dir_block(inode, f, &block, inode_index, i) > 0) {
...
int save_inode_dir_blocks(int inode_index, Directory_block* blocks){
	...
	int sum = 0;
	while (current != NULL){
		sprintf(filename, "tmp/dirblocks%dI%d.tmp", (++sum), inode_index);
		f = fopen(filename, "w+");
		if (f == NULL){
			free(filename);
			return 1;
...
int save_dir_blocks(Directory_block* blocks){
	...
	int sum = 0;
	while (current != NULL){
		sprintf(filename, "tmp/dirblocks%d.tmp", (++sum));
		f = fopen(filename, "w+");
		if (f == NULL){
			free(filename);
			return 1;
		...
		fprintf(f, "%d\n", current->nb_index);
		for (int i = 0; i < current->nb_index; i++){
			save_index(i, sum, current->tab_index[i]);
		}
		current = current->next_block;
	}
	#if (DEBUG_MODE==1)
...
int read_dir_block(Disk* disk, FILE* f, Directory_block* block, int dir_index){
	...
	for (int i=0; i< block->nb_index; i++){
		sprintf(filename, "tmp/index%dDB%d.tmp", i, dir_index+1);
		...
		int inode_index = 0;
		rewind(index_file);
		fscanf(index_file, "%s %d", block->tab_index[i].name, &inode_index);
		fclose(index_file);
	
		block->tab_index[i].inode = &disk->inodes[inode_index];

		#if (DEBUG_MODE==1)
		printf("loaded index %d of disk dirblock %d\n", i, dir_index);
		printf("\tindex name: %s\n", block->tab_index[i].name);
		printf("\tsrc inode index: %d\n", disk->inodes[inode_index].index);
		printf("\tinode index: %d\n", block->tab_index[i].inode->index);
		#endif
	}
	free(filename);
	return 1;
...
int read_inode_dir_block(Inode* inode, FILE* f, Directory_block* block, int inode_index, int dir_index){
	...
	for (int i=0; i< block->nb_index; i++){
		sprintf(filename, "tmp/index%dI%dDB%d.tmp", i, inode_index, dir_index+1);
		index_file = fopen(filename, "r");
		if (index_file == NULL){
			free(filename);
			return 0;
		...
		rewind(index_file);
		fscanf(index_file, "%s", block->tab_index[i].name);
		fclose(index_file);
		
		block->tab_index[i].inode = inode;

		#if (DEBUG_MODE==1)
		printf("loaded index %d of dirblock %d of inode %d\n", i, dir_index, inode_index);
		printf("\tindex name: %s\n", block->tab_index[i].name);
		printf("\tinode index: %d\n", block->tab_index[i].inode->index);
		#endif
	}
...
int load_dir_blocks(Disk* disk, Directory_block** blocks, int nb_dir_blocks){
	...
	for (int i = 0; i < nb_dir_blocks; i++){
		sprintf(filename, "tmp/dirblocks%d.tmp", (i+1));
		f = fopen(filename, "r");
		if (f == NULL){
			free(filename);
			return 1;
		...
		if (read_dir_block(disk, f, &block, i) > 0) {
			addDirBlockTail(blocks, block);
		}
		sum = i+1;
		fclose(f);
...
int save_inodes(Inode* inodes){
	...
	current = inodes;
	while (current != NULL){
		sprintf(filename, "tmp/inodes%d.tmp", current->index);
		f = fopen(filename, "w+");
...
int read_inode(FILE* f, int inode_index, Inode* inode){
	...
	load_inode_dir_blocks(inode, inode_index, &inode->dir_blocks);
	// TODO: load_inode_data_blocks needs to be changed!
	//load_inode_data_blocks(inode_index, &inode->data_blocks, inode->nb_data_blocks);
	return 1;
}
...
int load_inodes(Inode** inodes, int nb_inodes){
	...
	for (int i = 0; i < nb_inodes; i++){
		sprintf(filename, "tmp/inodes%d.tmp", i);
		f = fopen(filename, "r");
...
int load_disk(Disk* disk){
	...
	if (fread(disk, sizeof(Disk*), 1, f) <= 0){
		printf("Error reading the disk content!\n");
		return 0;
	}
